...
The exported raw `handlePixelsMcpHttpRequest()` does not infer whether an embedding server is remote. It rejects declared or streamed bodies above 64 KiB. Remote embeddings must provide `authorize`, enforce rate limits at the server or reverse-proxy boundary, and retain the exact-origin policy. `startPixelsMcpHttpServer()` enforces the non-loopback authorization requirement for its own listener.
